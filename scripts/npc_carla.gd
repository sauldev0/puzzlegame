...
const CARLA_LVL_4 = preload("res://dialogues/Carla_lvl4.dialogue")
var is_player_close = false
var is_dialogue_active = false
@onready var label_name_carla = $LabelNameCarla
@onready var panel_dialogo_carla = $PanelDialogoCarla
...
func _process(delta):
	if is_player_close and Input.is_action_just_pressed("Interrogar") and not is_dialogue_active:
		DialogueManager.show_dialogue_balloon(CARLA_LVL_4, "start")
...
func on_dialogue_ended(dialogue):
	...
	if dialogue != CARLA_LVL_4:
		return
	await get_tree().create_timer(0.2).timeout
	panel_dialogo_carla.visible = true
	label_name_carla.visible = true
	var is_dialogue_active = false
