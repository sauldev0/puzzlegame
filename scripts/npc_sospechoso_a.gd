...
const SOSPECHOSO_A_LVL_8 = preload("res://dialogues/SospechosoA_lvl8.dialogue")
...
func _process(delta):
	if is_player_close and Input.is_action_just_pressed("Interrogar") and not is_dialogue_active:
		DialogueManager.show_dialogue_balloon(SOSPECHOSO_A_LVL_8, "start")
...
func on_dialogue_ended(dialogue):
	...
	if dialogue != SOSPECHOSO_A_LVL_8:
		return
