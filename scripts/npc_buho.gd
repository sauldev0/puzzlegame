...
const BUHO_LVL_6 = preload("res://dialogues/Buho_lvl6.dialogue")
var is_player_close = false
var is_dialogue_active = false
@onready var panel_dialogo_buho = $PanelDialogoBuho
@onready var label_name_buho = $LabelNameBuho
@onready var quiz_input_buho = $"../UID/QuizInputBuho"


func _ready():
	DialogueManager.dialogue_started.connect(on_dialogue_started)
	DialogueManager.dialogue_ended.connect(on_dialogue_ended)
	quiz_input_buho.quiz_resuelto.connect(_on_quiz_resuelto)
	
func _process(delta):
	if is_player_close and Input.is_action_just_pressed("Interrogar") and not is_dialogue_active:
		DialogueManager.show_dialogue_balloon(BUHO_LVL_6, "start")
...
func on_dialogue_ended(dialogue):
	...
	if dialogue != BUHO_LVL_6:
		return
	await get_tree().create_timer(0.2).timeout
	
	if not LEVELCORE.quiz_ya_resuelto(npc_id):
		quiz_input_buho.npc_id = npc_id
		quiz_input_buho.visible = true
	
	print("Resueltos:", LEVELCORE.quiz_resueltos)
	panel_dialogo_buho.visible = true
	label_name_buho.visible = true
	var is_dialogue_active = false
