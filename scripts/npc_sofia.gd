...
const SOFIA_LVL_4 = preload("res://dialogues/Sofia_lvl4.dialogue")
var is_player_close = false
var is_dialogue_active = false
@onready var label_name_sofia = $LabelNameSofia
@onready var panel_dialogo_sofia = $PanelDialogoSofia
...
func _process(delta):
	if is_player_close and Input.is_action_just_pressed("Interrogar") and not is_dialogue_active:
		DialogueManager.show_dialogue_balloon(SOFIA_LVL_4, "start")
...
func on_dialogue_ended(dialogue):
	...
	if dialogue != SOFIA_LVL_4:
		return
	await get_tree().create_timer(0.2).timeout
	panel_dialogo_sofia.visible = true
	label_name_sofia.visible = true
	var is_dialogue_active = false
