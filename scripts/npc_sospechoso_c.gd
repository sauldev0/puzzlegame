...
@onready var label_name_c = $LabelNameC



const SOSPECHOSO_C_LVL_8 = preload("res://dialogues/SospechosoC_lvl8.dialogue")
...
func _process(delta):
	if is_player_close and Input.is_action_just_pressed("Interrogar") and not is_dialogue_active:
		DialogueManager.show_dialogue_balloon(SOSPECHOSO_C_LVL_8, "start")
...
func on_dialogue_ended(dialogue):
	...
	if dialogue != SOSPECHOSO_C_LVL_8:
		return
	await get_tree().create_timer(0.2).timeout
	panel_dialogo_c.visible = true
	label_name_c.visible = true
	var is_dialogue_active = false
